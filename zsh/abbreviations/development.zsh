...
abbr -S llip='ll-issues path'
abbr -S llig='gloww "$(ll-issues path )"'
abbr -S llsq='ll-issues sequence'
abbr -S lll='ll-loop'
...
abbr -S llilop='ll-issues list --json | jq -r '\''[.[] | select(.type == "EPIC" | not) | .id] | join(",")'\'''
