...
Automatically sync your recent GitHub repositories to your Obsidian vault for display in your Command Center.

## ▸ Overview

The GitHub sync system fetches your most recent repositories using the `gh` CLI and generates a Dataview-compatible markdown file in your vault.

### Features

-  Daily automated sync via launchd (7:00 AM)
-  Runs on login for fresh data
-  Private repo support (uses `gh` CLI auth)
-  Dataview-compatible frontmatter
-  Offline resilience (never overwrites on failure)

### Output

Generated file: `MC-vault/_Vault/_Data/github-repos.md`

```markdown
| Repository | Description | Language | Last Push | Visibility |
|------------|-------------|----------|-----------|------------|
| [dotfiles](url) | Personal configs... | Shell | Nov 24 |  |
| [project-x](url) | API server... | TypeScript | Nov 23 |  |
```
...
2. **Obsidian vault** at `~/AIProjects/MC-vault/`

### Install the Sync Service

```bash
cd ~/AIProjects/ai-workspaces/dotfiles
./macos/install-github-sync-service.sh
```

This installs a launchd service that:
- Syncs daily at 7:00 AM
- Syncs once on login
- Logs to `/tmp/github-sync.log`
...
```bash
~/AIProjects/MC-vault/_Vault/scripts/sync-github-repos.sh
```

Or trigger the service:

```bash
launchctl start com.vault.github-sync
```
...
Add this to your Command Center or any note:
...
TABLE WITHOUT ID
  file.link as "Repos",
  repo_count as "Count",
  updated as "Last Sync"
FROM "_Vault/_Data"
WHERE type = "github-repos"
```
...
|----------|---------|---------|
| `GH_REPO_LIMIT` | `15` | Number of repos to fetch |
| `OUTPUT_FILE` | `_Vault/_Data/github-repos.md` | Output location |

Example with custom limit:

```bash
GH_REPO_LIMIT=25 ~/AIProjects/MC-vault/_Vault/scripts/sync-github-repos.sh
```

### Modify Schedule

Edit the plist to change sync time:

```bash
# Edit the installed plist
nano ~/Library/LaunchAgents/com.vault.github-sync.plist

# Reload after changes
launchctl unload ~/Library/LaunchAgents/com.vault.github-sync.plist
launchctl load ~/Library/LaunchAgents/com.vault.github-sync.plist
```
...
# Errors
tail -f /tmp/github-sync.error.log
```
...
```bash
# Stop service
launchctl stop com.vault.github-sync
...
# Remove completely
launchctl unload ~/Library/LaunchAgents/com.vault.github-sync.plist
rm ~/Library/LaunchAgents/com.vault.github-sync.plist
```
...
- Check that `_Vault/_Data/` directory exists
...
dotfiles/
├── macos/
│   ├── com.vault.github-sync.plist      ← launchd config
│   └── install-github-sync-service.sh   ← Service installer
└── obsidian/
    └── README.md                         ← This file
...
└── _Vault/
    ├── _Data/
    │   └── github-repos.md               ← Generated output
    └── scripts/
        └── sync-github-repos.sh          ← Sync script
```
